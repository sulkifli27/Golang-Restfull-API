...
func setupTestDB() *sql.DB {
	db, err := sql.Open("mysql", "root@tcp(localhost:3306)/golang_restfullapi_test")
...
func TestCreateCategorySuccess(t *testing.T) {
	...
	assert.Equal(t, "Berhasil", responseBody["status"])
...
func TestDeleteCategorySuccess(t *testing.T) {
	...
	assert.Equal(t, 200, int(responseBody["code"].(float64)))
	assert.Equal(t, "Berhasil Delete", responseBody["status"])
}
